...
> TL;DR: Hilt 2.59.2 (KSP) jako jedyny mechanizm DI.
...
`com.google.dagger:hilt-android:2.59.2` + plugin `com.google.dagger.hilt.android` + KSP `hilt-compiler`. `enableAggregatingTask = true` dla incremental build. `@HiltViewModel` w `ui/`, `@Module @InstallIn(SingletonComponent::class)` w `data/`.
