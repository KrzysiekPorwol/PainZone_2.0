...
- (zrobione) **M4.3 · StatsExerciseScreen** — S10 lista serii grupowana per sesja (header `DD.MM · Plan · Dzień`, wiersz `reps × kg · RPE · po Xs odpocz.`, 1. seria rest `—`) + segmentowany filtr 30d/90d/Rok/Wszystko (default 90d, `flatMapLatest` re-subskrybuje query). S3 (Postęp) jako entry point: lista aktywnych ćwiczeń → tap S10. Nazwa+grupa niesione w route (jak `DayDetail`) → tytuł przeżyje soft-delete (M4.5). `ostatnio Xd` i filtrowanie ćwiczeń-bez-historii na S3 odłożone (wymaga agregatu sesji). Best-set highlight → M4.4, marker usunięte → M4.5.
...
- **M5.5 · App icon** — adaptive icon (foreground + background) + Play Store icon.
- **M5.6 · Play Store assets** — screenshoty, opis (PL), feature graphic, kategoria.
- **M5.7 · Release build** — signing config, R8/proguard rules, Internal testing track upload.
